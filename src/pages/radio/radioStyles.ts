...
.radio-theme-cloud { display: flex; flex-wrap: wrap; justify-content: center; gap: 9px; padding: 4px clamp(4px,1vw,12px); background: radial-gradient(ellipse at center,rgba(234,76,137,.04),transparent 68%); }
.radio-theme { position: relative; overflow: hidden; display: inline-grid; grid-template-columns: auto auto; align-items: center; gap: 10px; min-height: 58px; border: 1px solid color-mix(in srgb, var(--radio-accent) 22%, white); border-radius: 999px; padding: 8px 18px 8px 9px; color: var(--radio-ink); text-align: left; background: rgba(255,255,255,.88); box-shadow: 0 8px 20px rgba(23,21,43,.04); }
...
.radio-theme__copy { display: grid; gap: 2px; }.radio-theme__copy strong { font-size: 14px; }.radio-theme__copy small { color: var(--muted); font-size: 11px; }
...
@media (max-width: 980px) { .radio-hero { grid-template-columns: 1fr; }.radio-console { justify-self: center; }.radio-create-stage { grid-template-columns: 150px 1fr; }.radio-generate { grid-column: 1 / -1; justify-self: stretch; justify-content: center; } }
@media (max-width: 640px) { .radio-page { gap: 18px; }.radio-hero { min-height: auto; border-radius: 20px; padding: 24px 18px; }.radio-hero h1 { font-size: clamp(32px,10vw,44px); white-space: normal; }.radio-live-line { margin-bottom: 20px; }.radio-now-meta { grid-template-columns: 1fr; }.radio-console { padding: 13px; transform: none; }.radio-display { min-height: 130px; }.radio-console__hardware { grid-template-columns: 1fr 76px; min-height: 90px; }.radio-knob { width: 58px; height: 58px; }.radio-section-heading { align-items: start; }.radio-section-heading h2 { font-size: 26px; }.radio-tuner { grid-template-columns: 1fr; padding: 14px; border-radius: 14px; }.radio-tuner__knob { display: none; }.radio-tuner__numbers span:nth-child(even) { display: none; }.radio-tuner__caption { grid-template-columns: auto 1fr; }.radio-tuner__caption small { display: none; }.radio-theme-cloud { justify-content: flex-start; gap: 8px; padding: 6px 0; }.radio-theme,.radio-theme:nth-child(n) { min-height: 50px; padding: 6px 12px 6px 7px; transform: none; }.radio-theme__icon { width: 36px; height: 36px; }.radio-create-stage { grid-template-columns: 1fr; border-radius: 22px; padding: 22px 18px; }.radio-create-visual { width: 138px; }.radio-generate { grid-column: auto; grid-template-columns: auto 1fr auto; padding-right: 14px; }.radio-generate__wave { width: 46px; height: 46px; } }
`
