...
import type { AdminChallenge, AdminComment, AdminData, AdminInvite, AdminSong, AdminStats, AdminUser } from './types'
...
type BackendComment={id:string;songId:string;songTitle:string;userId:string|null;userName:string;anon:boolean;text:string;createdAt:string}
...
const mapComment=(c:BackendComment):AdminComment=>({id:c.id,songId:c.songId,songTitle:c.songTitle,userId:c.userId,name:c.userName,anon:c.anon,text:c.text,t:toTime(c.createdAt)})
...
export async function all():Promise<AdminData>{
 const [summary,users,invites,songs,comments,challenges]=await Promise.all([request<BackendStats>('/api/admin/stats'),request<ListResponse<BackendUser>>('/api/admin/users?pageSize=1000'),request<ListResponse<BackendInvite>>('/api/admin/invites'),request<ListResponse<BackendSong>>('/api/admin/songs?pageSize=1000'),request<ListResponse<BackendComment>>('/api/admin/comments?pageSize=1000'),request<ListResponse<BackendChallenge>>('/api/admin/challenges')])
 return{summary:mapStats(summary),users:users.list.map(mapUser),invites:invites.list.map(mapInvite),songs:songs.list.map(mapSong),comments:comments.list.map(mapComment),challenges:challenges.list.map(mapChallenge)}
}
